...
describe Raml::Method do
  ...
  describe '#new' do
    ...
    context 'when the securedBy property is missing' do
      let(:data) { { } }
      it { expect{ subject }.to_not raise_error }
      it { expect(subject.secured_by).to be_empty }
    end
    ...
    context 'when the securedBy property is defined' do
      ...
      context 'when the securedBy property is included it is accessible and' do
        ...
        it 'exposes the schema references' do
          expect( subject.secured_by ).to all( be_a Raml::SecuritySchemeReference )
          subject.secured_by.map(&:name).should contain_exactly('oauth_2_0', 'oauth_1_0')
        end
      end
    end
  end
